...
public class Graph {
    ...
    // ---------- NODE ------------
    /**
     * Add a new node to the graph. The node automatically gets assigned an int id, with collisions handled
    ...
    public int addNode(String label) {
        if(ids.containsKey(label)) return -1;
        //assign an id automatically
        int offset = 0;
        int id = num_nodes;
        do {
            id = num_nodes + Math.pow(offset++, 2);
    ...
    public void addNode(int id) {
        ...
        do {
             label = Integer.toHexString(id + Math.pow(i++, 2));
